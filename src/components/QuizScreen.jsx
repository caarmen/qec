...
function getFeedbackMessage(
  ...
  return t(
    isCorrect
      ? "quizScreen.feedback.correctAnswer"
      : "quizScreen.feedback.incorrectAnswer",
  );
}
...
function getSelectedAnswers(
  ...
  if (multipleCorrectAnswers) {
    ...
    if (currentSelectedAnswerIds.includes(selectedAnswerId)) {
      return currentSelectedAnswerIds.filter(
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
      <QuestionCard className="w-full max-w-md">
        ...
        <div className="flex justify-between items-center text-sm text-gray-600">
          <p tabIndex={-1} ref={progressRef}>
            {t("quizScreen.progress", {
            ...
            return (
              <li key={answer.id}>
                <AnswerOption
                  ...
                  text={answer.text}
                  role={
                    difficulty === DIFFICULTY.DIFFICULT ? "checkbox" : "radio"
                  }
                  isSelected={selectedAnswers.includes(answer.id)}
                  onSelect={() =>
                    onSelectAnswer(
                      getSelectedAnswers(selectedAnswers, answer.id, {
                        multipleCorrectAnswers:
                          difficulty === DIFFICULTY.DIFFICULT,
                      }),
                    )
        ...
        >
          {t(
            hasAnswerSubmitted
              ? "quizScreen.buttonNext"
              : "quizScreen.buttonSubmit",
          )}
        </Button>
      </QuestionCard>
    </div>
