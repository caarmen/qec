...
describe("Edge Cases", () => {
  ...
  describe("score calculation edge cases", () => {
    ...
    it("should display correct count of answered questions", async () => {
      ...
      const correctText =
        screen.getByText(/réponses correctes :/i).parentElement.textContent;
  ...
  describe("navigation edge cases", () => {
    ...
    it("should not show results screen during quiz", async () => {
      ...
      expect(
        screen.queryByRole("heading", { name: /quiz terminé/i }),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: /recommencer le quiz/i }),
      ).not.toBeInTheDocument();
    });
    ...
    it("should not show start or quiz screen after completion", async () => {
      ...
      expect(
        screen.queryByRole("button", { name: /^commencer le quiz/i }),
      ).not.toBeInTheDocument();

      // Should not show quiz screen elements
      expect(
        screen.queryByText(/question \d+ sur \d+/i),
      ).not.toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: /suivant/i }),
      ).not.toBeInTheDocument();
  ...
  // Set a higher timeout of 12 seconds (5 seconds in the default) for these integration tests
}, 12000);
